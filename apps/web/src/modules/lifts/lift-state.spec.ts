...
import { State, reducer, Actions } from "./lift-state.ts";
...
it("initialising does nothing if already created", () => {
  const state: State = { type: "created", lowestFloor: 1, highestFloor: 10, currentFloor: 4 };
...
it("transits to the given floor", () => {
  const state: State = { type: "created", lowestFloor: 1, highestFloor: 10, currentFloor: 1 };
